...
from RepairSinglePattern3 import find_instances
...
def prepare_data(patterns,df,rules):
    ...
    for x in patterns:
        print(x)
        if not 'interleaving' in [f for xs in x for f in xs]:
            bar.next()
            if len(x) == 1:
                occorrenze.append(count_occurences(df,x[0]))
            else:
                graph = count_occurences_multisubs(x, rules, len(df))
                # graph = check_graphlist_patterns([g.replace('Grafo', 'graph') for g in graph],
                #                          ['Sub_' + str(sub) for sub in x if not isinstance(sub, list)],
                #                          'ProcessRepairing/patterns_file/')
                # graph = [g.replace('graph', 'Grafo') for g in graph]
                occorrenze.append(count_oc(df,graph))
        else:
            occorrenze.append([0]*len(df))
...
def main(path0, datasetname):
    ...
    df = pd.read_csv(path + datasetname + "_table2_on_file.csv",
                                                 sep=";")
    patterns = create_patterns_list(path + datasetname + "_new_patterns.subs")
...
parser.add_argument("path", type=str, help="Path of the dir containing: *_table2_on_file.csv | *_new_patterns.subs | rules_log.txt")
